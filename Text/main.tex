...
\begin{adjustbox}{width=\linewidth}
	\begin{tabular}{|l|l|c|l|}
		...
		Автор попытки   & A\_AUTHOR  &    N(4)    & внешний ключ (к User)               \\ \hline
		Проверяющий     & A\_CHECKER &    N(4)    & внешний ключ (к User)               \\ \hline
...
\textbf{1НФ}
Для приведения к первой нормальной форме значение многозначного поля «Категории» в отошении КУРС было вынесено в отдельное справочное отношение. Помимо этого поле «ФИО» в отношении ПОЛЬЗОВАЕТЛЬ было разделено на 3 поля: обязательное «Фамилия», обязательное «Имя» и необязательное «Отчество».
...
\textbf{3НФ}
Для приведение к 3НФ необходимо выявить транзитивные зависимости. Транзитивная зависимость выяалена для «Продолжительность» из отношения ЛЕКЦИЯ. Продолжительность лекции зависит от объема информации в поле «Содержание», значит можно убрать поле «Продолжительность» и создать представление, отражающие время изучения лекции.
...
\subsection{Полученные после нормализации отншения}
...
\begin{table}[H]
	\begin{flushleft} 
		\tablecaption{\label{tab:UserV2} Схема отношения ПОЛЬЗОВАТЕЛЬ (User) }
	\end{flushleft}
	\begin{adjustbox}{width=\linewidth}
		\begin{tabular}{|l|l|c|l|}
			\hline
